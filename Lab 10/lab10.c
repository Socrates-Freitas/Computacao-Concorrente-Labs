...
#define NUM_LEITORAS 2
#define NUM_ESCRITORAS 1

// Globais
sem_t em_e, em_l, escr, leit; //semaforos
int e=0, l=0, n_leit = 3; //globais

/*
    Se tem alguem escrevendo, diminui a fila de leitura.
...
void * leitores(void *args){
    ...
    while(1){
        ...
        sem_post(&em_l); // fim da exclusao mutua
        sem_post(&leit);
...
void * escritores(void *args){
    ...
    while(1){
        ...
        sem_post(&em_e); // fim da exclusao mutua
        sem_wait(&escr);
        ...
        e--;
        if(e == 0) { // se nao tem ninguem mais pra escrever...
            sem_post(&leit);
        }
        sem_post(&em_e);
    }
...
int main(){
    ...
    for(int i = 0; i < NUM_LEITORAS; i++){ // iniciando leitoras
        id[i] = i; 
